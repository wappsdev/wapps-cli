...
| `wapps secrets env --write <path>` | Writes one env file to disk (0600, atomic). Stdout stays empty. | One-off / ad-hoc path that isn't declared in `targets:`. |
...
| `wapps secrets sync` | Rebuilds the archive from declared sources; auto-applies targets. No values printed. | After someone added a new tofu output or edited `.env.shared` |
| `wapps secrets diff [ref]` | Shows added / changed / removed keys vs a git ref (default `HEAD~1`). Names only — never values. | After `git pull` to see what teammates changed. |
...
If `.wapps.yaml` declares `targets:` (the recommended setup), the
`predev` script handles it — `pnpm dev` triggers `wapps secrets apply`
automatically. If you're running the dev server manually:

```bash
# 1. Materialize every declared target (no stdout).
wapps secrets apply
...
For one-off or ad-hoc paths not declared in `targets:`:
...
After `set`, the archive, the file source declared in `.wapps.yaml`,
AND every declared target (e.g., `.env.local`) are updated atomically.
Commit the archive + file source; do not commit targets (they're
regenerated by `apply` on every dev session).
