...
- **`wapps secrets projects` moved to `wapps projects`.** A project is not a secrets-specific concept — the `--project` flag already lives at the root, and future non-secrets surfaces will name the same thing. The implementation stays in `cmd/secrets` (that is where the store plumbing lives, same as `dr`/`rotate`/`tofu`), but root-mounting means `SecretsCmd.PersistentPreRunE` no longer applies, so each leaf now guards itself: `list` stays agent-allowed (names only), `rm` stays control-plane. Neither checks the repo→project pin — both are global operations, like `policy` and `rotate-plan`.
...
### Changed
- **`worker/` moved to vitest 4 — the dev-dependency advisories are gone (0 vulnerabilities, down from 6 high).** They were all in the local test toolchain (esbuild's dev server, `sharp`/libvips and `undici` pulled in by miniflare) and none shipped to production, but the only fix was the upgrade: `@cloudflare/vitest-pool-workers 0.12 → 0.21`, `vitest 3 → 4`, `wrangler 4.72 → 4.123`, `workers-types 4 → 5`. Three v4 breaks had to be absorbed: `cloudflare:test` dropped its `fetchMock` export, `defineWorkersConfig` was replaced by a Vite plugin (`cloudflareTest`), and `ProvidedEnv` became `Cloudflare.Env`. All 189 tests pass unchanged.
- **`fetchMock` is now a local shim** (`worker/test/fetchmock.ts`, ~170 lines) carrying the exact undici surface these tests used, implemented over a global `fetch` stub. This keeps the upgrade from changing what the tests *mean*: every JWKS / get-identity / Discord / B2 intercept declaration stayed byte-for-byte. Stubbing the global is safe here because the tests never call it themselves — `callGate` invokes `worker.fetch()` directly and `identity.ts` takes an injectable fetcher — so only production code's outbound calls are captured.
- The two pool options v4 removed were re-established through vitest's own settings, because their original reasons still hold: `singleWorker: true` became `fileParallelism: false` + `isolate: false` (the JWKS signer is a singleton and the fetch mock holds module-scoped state — split across isolates, both silently stop working), and `isolatedStorage: false` needs no replacement since state was already reset per test in `resetWorld`.
